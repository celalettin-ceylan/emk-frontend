...
export default function DropdownsMenu({ menu }) {
    ...
    return (
        <div className="lg:relative lg:flex lg:justify-center lg:flex-row lg:text-lg"
            ...
            <div className={`bg-zinc-800 lg:absolute lg:border-[1px] lg:border-white lg:top-20 lg:z-20 lg:rounded-md ` + (isVisible ? " block" : " hidden")}>
                {menu.links.map(link =>
                    <Link href={link.uri} key={link.text}>
                        <a className="block text-white py-2 pl-6 lg:text-center lg:p-2 lg:m-2 lg:w-52 lg:rounded-md lg:hover:text-black lg:hover:bg-white">
                            {link.text}
                        </a>
                    </Link>
                )}
